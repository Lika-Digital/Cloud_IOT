...
def test_customer_list_messages(client, cust_headers, customer_id):
...
def test_customer_unread_count(client, cust_headers):
    r = client.get("/api/chat/unread-count", headers=cust_headers)
    assert r.status_code == 200
