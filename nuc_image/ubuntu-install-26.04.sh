#!/usr/bin/env bash
# ============================================================================
# Cloud IoT NUC v3.0 — Ubuntu Server 26.04 LTS (Resolute) Installer
#
# Differences from ubuntu-install.sh (24.04 variant):
#   • Detects + uses Python 3.14 (default on 26.04) or 3.13 if present
#   • Installs Rust toolchain + extra -dev libs so packages without 3.14
#     wheels (pydantic-core, etc.) can build from source
#   • Auto-detects Ubuntu codename for Docker repo, falls back to noble
#   • Generates a relaxed requirements list for Python >= 3.13 so pinned
#     numpy / pydantic / scikit-learn don't force impossible source builds
#   • Removes `-q` from pip so progress is visible (no more "stuck" feeling)
#
# Usage:
...
[[ $EUID -ne 0 ]] && error "Run with sudo: sudo bash nuc_image/ubuntu-install-26.04.sh"
...
info "Detected: ${PRETTY_NAME:-unknown} (codename: ${UBUNTU_CODENAME})"
...
cat << 'BANNER'
  ...
  ║       Ubuntu Server 26.04 LTS (Resolute) Installer  v3.0       ║
  ╚══════════════════════════════════════════════════════════════════╝
BANNER
echo -e "${NC}"
echo -e "  Repo: ${WHITE}${REPO_DIR}${NC}"
echo -e "  Log:  ${WHITE}${LOG_FILE}${NC}"
echo -e "  OS:   ${WHITE}${PRETTY_NAME:-unknown}${NC}"
echo ""
echo "  Internet connection (ethernet) required."
echo "  First-build on a new Ubuntu may take 20-30 minutes due to source builds."
...
phase "Installing system packages (incl. Rust toolchain + build deps for source builds)"
export DEBIAN_FRONTEND=noninteractive
apt-get update -qq
# Base dev tooling + headers required when packages have no Python 3.14 wheels yet.
# Rust (rustc/cargo) is needed by pydantic-core and a few other PyO3 packages.
...
if ! command -v docker &>/dev/null; then
  ...
  # Auto-detect Docker codename. Docker may not have published for
  # very-fresh Ubuntu releases yet — fall back to noble (24.04 LTS) which
  ...
  if ! curl -fsI "https://download.docker.com/linux/ubuntu/dists/${UBUNTU_CODENAME}/Release" >/dev/null 2>&1; then
    warn "Docker repo has no '${UBUNTU_CODENAME}' suite yet — falling back to 'noble'"
...
PYTHON_BIN="python3"
# Prefer the newest interpreter available — wheels lag behind, but pydantic /
# numpy / scikit-learn floor pins below pick the latest 3.14-compatible version.
for v in python3.14 python3.13 python3.12 python3.11; do
  command -v "$v" &>/dev/null && PYTHON_BIN="$v" && break
done
PY_VERSION_FULL=$("$PYTHON_BIN" --version 2>&1 | awk '{print $2}')
PY_MAJOR_MINOR=$(echo "$PY_VERSION_FULL" | cut -d. -f1,2)
info "Using: $PYTHON_BIN ($PY_VERSION_FULL)"
...
# On Python >= 3.13, pinned numpy 2.1 / pydantic 2.9 / scikit-learn 1.5 may have
# no wheels — relax to floor pins so pip picks the latest compatible release.
REQ_SRC="${APP_DIR}/backend/requirements.txt"
REQ_RUN="${APP_DIR}/backend/requirements.runtime.txt"

# String comparison works for major.minor ≤ 3.99
if [[ "$(printf '%s\n' '3.13' "$PY_MAJOR_MINOR" | sort -V | head -1)" == "3.13" ]]; then
  info "Python ${PY_MAJOR_MINOR} detected — relaxing numpy / pydantic / scikit-learn pins"
  sed -E \
    -e 's/^numpy==.*/numpy>=2.2/' \
...
echo ""
echo -e "${YELLOW}  Installing Python packages — this can take 10-20 min on a new"
echo -e "  Ubuntu release (some packages compile from source against ${PY_MAJOR_MINOR}).${NC}"
echo -e "  Progress shown below:"
echo ""
...
# OS: ${PRETTY_NAME:-unknown}    Python: ${PY_VERSION_FULL}
...
SIMEOF
    ...
    "${VENV}/bin/pip" install --prefer-binary -r "${APP_DIR}/backend/requirements.txt"
    ...
    ;;
  version)
    echo "Cloud IoT NUC v3.0 (26.04 build)"
...
cat << 'DONE'
  ...
  ║              (Ubuntu 26.04 / Python 3.14 build)                 ║
  ╚══════════════════════════════════════════════════════════════════╝
DONE
echo -e "${NC}"
...
echo -e "  ${WHITE}Python    :${NC}  ${PY_VERSION_FULL}"
echo -e "  ${WHITE}OS        :${NC}  ${PRETTY_NAME:-unknown}"
